# caedqn/anneal_train.py
"""
Training script modified to log the average CAE exploration bonus per episode.
This is used to demonstrate the self‑annealing property (RQ3).
Run for a shorter number of episodes (e.g., 20,000) to capture the bonus decay.
"""
...
HEIGHT = 6
N_MINES = 4
EPISODES = 20_000          # shorter run to capture bonus decay
ENV_SEED = 2004
MODEL_SEED = 123
PRINT_EVERY = 1000
MODEL_NAME = "caedqn_anneal"
...
def main():
    ...
    print("=" * 60)
    print("Algorithm: CAE-DQN (Critic as Explorer) - Bonus Annealing Run")
    ...
    csv_filename = "csv/training_caedqn_anneal.csv"
    csv_file = open(csv_filename, 'w', newline='')
    csv_writer = csv.writer(csv_file)
    # Added 'avg_bonus' column
    csv_writer.writerow(['episode', 'total_reward', 'progress', 'win', 'n_clicks',
                         'beta', 'learn_rate', 'avg_bonus'])

    eval_csv_filename = "csv/training_eval_caedqn_anneal.csv"
    eval_csv_file = open(eval_csv_filename, 'w', newline='')
    eval_csv_writer = csv.writer(eval_csv_file)
    eval_csv_writer.writerow(['episode', 'med_progress', 'win_rate', 'med_reward',
                              'beta', 'learn_rate', 'time_elapsed', 'avg_bonus_sma'])

    episode_rewards = []
    episode_progress = []
    episode_wins = []
    episode_avg_bonuses = []   # store average bonus per episode
    ...
    for episode in range(1, EPISODES + 1):
        ...
        step_bonuses = []      # collect bonuses for this episode
        ...
        while not done:
            ...
            # Compute CAE exploration bonus
            bonus = agent.compute_bonus(new_state) if not done else 0
            augmented_reward = reward + bonus

            episode_reward += augmented_reward
            step_bonuses.append(bonus)
        ...
        avg_bonus = np.mean(step_bonuses) if step_bonuses else 0.0
        episode_avg_bonuses.append(avg_bonus)
        ...
        if episode % PRINT_EVERY == 0:
            ...
            # Smooth average bonus over the last PRINT_EVERY episodes
            recent_bonuses = episode_avg_bonuses[-PRINT_EVERY:]
            avg_bonus_sma = round(np.mean(recent_bonuses), 4) if recent_bonuses else 0.0

            print(f"Episode {episode}/{EPISODES} | "
                  f"Med progress: {med_progress} | "
                  f"Win rate: {win_rate} | "
                  f"Med reward: {med_reward} | "
                  f"Avg Bonus (SMA): {avg_bonus_sma:.4f} | "
